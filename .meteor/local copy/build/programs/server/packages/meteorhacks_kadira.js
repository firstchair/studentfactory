...
WaitTimeBuilder.prototype.register = function(session, msgId) {                                                       // 10
  ...
  var waitList =                                                                                                      // 14
    session.inQueue &&                                                                                                // 15
    session.inQueue.map &&                                                                                            // 16
    session.inQueue.map(function(msg) {                                                                               // 17
    var key = self._getMessageKey(session.id, msg.id);                                                                // 18
    return self._getCacheMessage(key, msg);                                                                           // 19
  });                                                                                                                 // 20
                                                                                                                      // 21
  waitList = waitList || [];                                                                                          // 22
                                                                                                                      // 23
  //add currently processing ddp message if exists                                                                    // 24
  var currentlyProcessingMessage = this._currentProcessingMessages[session.id];                                       // 25
  if(currentlyProcessingMessage) {                                                                                    // 26
    var key = self._getMessageKey(session.id, currentlyProcessingMessage.id);                                         // 27
    waitList.unshift(this._getCacheMessage(key, currentlyProcessingMessage));                                         // 28
  }                                                                                                                   // 29
                                                                                                                      // 30
  this._waitListStore[mainKey] = waitList;                                                                            // 31
};                                                                                                                    // 32
                                                                                                                      // 33
WaitTimeBuilder.prototype.build = function(session, msgId) {                                                          // 34
  var mainKey = this._getMessageKey(session.id, msgId);                                                               // 35
  var waitList = this._waitListStore[mainKey] || [];                                                                  // 36
  delete this._waitListStore[mainKey];                                                                                // 37
                                                                                                                      // 38
  var filteredWaitList =  waitList.map(this._cleanCacheMessage.bind(this));                                           // 39
  return filteredWaitList;                                                                                            // 40
};                                                                                                                    // 41
                                                                                                                      // 42
WaitTimeBuilder.prototype._getMessageKey = function(sessionId, msgId) {                                               // 43
  return sessionId + "::" + msgId;                                                                                    // 44
};                                                                                                                    // 45
                                                                                                                      // 46
WaitTimeBuilder.prototype._getCacheMessage = function(key, msg) {                                                     // 47
  var self = this;                                                                                                    // 48
  var cachedMessage = self._messageCache[key];                                                                        // 49
  if(!cachedMessage) {                                                                                                // 50
    self._messageCache[key] = cachedMessage = _.pick(msg, WAITON_MESSAGE_FIELDS);                                     // 51
    cachedMessage._key = key;                                                                                         // 52
    cachedMessage._registered = 1;                                                                                    // 53
  } else {                                                                                                            // 54
    cachedMessage._registered++;                                                                                      // 55
  }                                                                                                                   // 56
                                                                                                                      // 57
  return cachedMessage;                                                                                               // 58
};                                                                                                                    // 59
                                                                                                                      // 60
WaitTimeBuilder.prototype._cleanCacheMessage = function(msg) {                                                        // 61
  msg._registered--;                                                                                                  // 62
  if(msg._registered == 0) {                                                                                          // 63
    delete this._messageCache[msg._key];                                                                              // 64
  }                                                                                                                   // 65
                                                                                                                      // 66
  // need to send a clean set of objects                                                                              // 67
  // otherwise register can go with this                                                                              // 68
  return _.pick(msg, WAITON_MESSAGE_FIELDS);                                                                          // 69
};                                                                                                                    // 70
                                                                                                                      // 71
WaitTimeBuilder.prototype.trackWaitTime = function(session, msg, unblock) {                                           // 72
  var self = this;                                                                                                    // 73
  var started = Date.now();                                                                                           // 74
  self._currentProcessingMessages[session.id] = msg;                                                                  // 75
                                                                                                                      // 76
  var unblocked = false;                                                                                              // 77
  var wrappedUnblock = function() {                                                                                   // 78
    if(!unblocked) {                                                                                                  // 79
      var waitTime = Date.now() - started;                                                                            // 80
      var key = self._getMessageKey(session.id, msg.id);                                                              // 81
      var cachedMessage = self._messageCache[key];                                                                    // 82
      if(cachedMessage) {                                                                                             // 83
        cachedMessage.waitTime = waitTime;                                                                            // 84
      }                                                                                                               // 85
      delete self._currentProcessingMessages[session.id];                                                             // 86
      unblocked = true;                                                                                               // 87
      unblock();                                                                                                      // 88
    }                                                                                                                 // 89
  };                                                                                                                  // 90
                                                                                                                      // 91
  return wrappedUnblock;                                                                                              // 92
};                                                                                                                    // 93
////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
    ...
    Kadira._postData(endpoint, httpOptions, function(err, response){                                                  // 214
      ...
      if(response && response.statusCode == 200) {                                                                    // 220
        ...
        tryAgain(err);                                                                                                // 227
                                                                                                                      ...
                                                                                                                      // 231
  function tryAgain(err) {                                                                                            // 232
    err = err || {};                                                                                                  // 233
    logger('retrying to send payload to server')                                                                      // 234
    if(++payloadRetries < 5) {                                                                                        // 235
      payloadRetry.retryLater(payloadRetries, callHTTP);                                                              // 236
    } else {                                                                                                          // 237
      cleaPayloadRetry();                                                                                             // 238
      callback && callback(err);                                                                                      // 239
    }                                                                                                                 // 240
  }                                                                                                                   // 241
                                                                                                                      // 242
  function cleaPayloadRetry() {                                                                                       // 243
    payloadRetries = 0;                                                                                               // 244
    payloadRetry.clear();                                                                                             // 245
  }                                                                                                                   // 246
}                                                                                                                     // 247
                                                                                                                      // 248
// this return the __kadiraInfo from the current Fiber by default                                                     // 249
// if called with 2nd argument as true, it will get the kadira info from                                              // 250
// Meteor.EnvironmentVariable                                                                                         // 251
//                                                                                                                    // 252
// WARNNING: returned info object is the reference object.                                                            // 253
//  Changing it might cause issues when building traces. So use with care                                             // 254
Kadira._getInfo = function(currentFiber, useEnvironmentVariable) {                                                    // 255
  currentFiber = currentFiber || Fibers.current;                                                                      // 256
  if(currentFiber) {                                                                                                  // 257
    if(useEnvironmentVariable) {                                                                                      // 258
      return Kadira.env.kadiraInfo.get();                                                                             // 259
    }                                                                                                                 // 260
    return currentFiber.__kadiraInfo;                                                                                 // 261
  }                                                                                                                   // 262
};                                                                                                                    // 263
                                                                                                                      // 264
// this does not clone the info object. So, use with care                                                             // 265
Kadira._setInfo = function(info) {                                                                                    // 266
  Fibers.current.__kadiraInfo = info;                                                                                 // 267
};                                                                                                                    // 268
                                                                                                                      // 269
Kadira.enableErrorTracking = function () {                                                                            // 270
  __meteor_runtime_config__.kadira.enableErrorTracking = true;                                                        // 271
  Kadira.options.enableErrorTracking = true;                                                                          // 272
};                                                                                                                    // 273
                                                                                                                      // 274
Kadira.disableErrorTracking = function () {                                                                           // 275
  __meteor_runtime_config__.kadira.enableErrorTracking = false;                                                       // 276
  Kadira.options.enableErrorTracking = false;                                                                         // 277
};                                                                                                                    // 278
                                                                                                                      // 279
Kadira.trackError = function (type, message, options) {                                                               // 280
  if(Kadira.options.enableErrorTracking && type && message) {                                                         // 281
    options = options || {};                                                                                          // 282
    options.subType = options.subType || 'server';                                                                    // 283
    options.stacks = options.stacks || '';                                                                            // 284
    var error = {message: message, stack: options.stacks};                                                            // 285
    var trace = {                                                                                                     // 286
      type: type,                                                                                                     // 287
      subType: options.subType,                                                                                       // 288
      name: message,                                                                                                  // 289
      errored: true,                                                                                                  // 290
      at: Kadira.syncedDate.getTime(),                                                                                // 291
      events: [['start', 0, {}], ['error', 0, {error: error}]],                                                       // 292
      metrics: {total: 0}                                                                                             // 293
    };                                                                                                                // 294
    Kadira.models.error.trackError(error, trace);                                                                     // 295
  }                                                                                                                   // 296
}                                                                                                                     // 297
                                                                                                                      // 298
Kadira.ignoreErrorTracking = function (err) {                                                                         // 299
  err._skipKadira = true;                                                                                             // 300
}                                                                                                                     // 301
                                                                                                                      // 302
Kadira._postData = function (endpoint, options, callback) {                                                           // 303
  var content = JSON.stringify(options.data);                                                                         // 304
                                                                                                                      // 305
  var headers = options.headers;                                                                                      // 306
  headers['Content-Type'] = 'application/json';                                                                       // 307
                                                                                                                      // 308
  var options = {                                                                                                     // 309
    url: endpoint,                                                                                                    // 310
    method: 'POST',                                                                                                   // 311
    ...
    headers: headers                                                                                                  // 314
                                                                                                                      ...
                                                                                                                      // 316
  if(Kadira.options.proxy) {                                                                                          // 317
    options.proxy = Kadira.options.proxy;                                                                             // 318
  }                                                                                                                   // 319
                                                                                                                      // 320
  request(options, function (error, res, body) {                                                                      // 321
    if(error) {                                                                                                       // 322
      console.error('Kadira:', error.message);                                                                        // 323
      return callback(error);                                                                                         // 324
    }                                                                                                                 // 325
                                                                                                                      // 326
    var response = {};                                                                                                // 327
    response.statusCode = res.statusCode;                                                                             // 328
    callback(null, response);                                                                                         // 329
  });                                                                                                                 // 330
}                                                                                                                     // 331
////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
        ...
        if(msg.error) {                                                                                               // 100
          ...
          if(kadiraInfo && kadiraInfo.currentError) {                                                                 // 104
            ...
            error = _.pick(kadiraInfo.currentError, ['message', 'stack']);                                            // 107
                                                                                                                      ...
                                                                                                                      // 109
          Kadira.tracer.endLastEvent(kadiraInfo.trace);                                                               // 110
          Kadira.tracer.event(kadiraInfo.trace, 'error', {error: error});                                             // 111
        } else {                                                                                                      // 112
          var isForced = Kadira.tracer.endLastEvent(kadiraInfo.trace);                                                // 113
          if (isForced) {                                                                                             // 114
            console.warn('Kadira endevent forced complete', JSON.stringify(kadiraInfo.trace.events));                 // 115
          };                                                                                                          // 116
          Kadira.tracer.event(kadiraInfo.trace, 'complete');                                                          // 117
        }                                                                                                             // 118
                                                                                                                      // 119
        //processing the message                                                                                      // 120
        var trace = Kadira.tracer.buildTrace(kadiraInfo.trace);                                                       // 121
        Kadira.models.methods.processMethod(trace);                                                                   // 122
                                                                                                                      // 123
        // error may or may not exist and error tracking can be disabled                                              // 124
        if(error && Kadira.options.enableErrorTracking) {                                                             // 125
          // if we have currentError, we should track that                                                            // 126
          // otherwise, try to get  error from DDP message                                                            // 127
          var errorToTrack = kadiraInfo.currentError || error;                                                        // 128
          Kadira.models.error.trackError(errorToTrack, trace);                                                        // 129
        }                                                                                                             // 130
                                                                                                                      // 131
        //clean and make sure, fiber is clean                                                                         // 132
        //not sure we need to do this, but a preventive measure                                                       // 133
        Kadira._setInfo(null);                                                                                        // 134
      }                                                                                                               // 135
    }                                                                                                                 // 136
                                                                                                                      // 137
    return originalSend.call(this, msg);                                                                              // 138
  };                                                                                                                  // 139
};                                                                                                                    // 140
                                                                                                                      // 141
// wrap existing method handlers for capturing errors                                                                 // 142
_.each(Meteor.default_server.method_handlers, function(handler, name) {                                               // 143
  wrapMethodHanderForErrors(name, handler, Meteor.default_server.method_handlers);                                    // 144
});                                                                                                                   // 145
                                                                                                                      // 146
// wrap future method handlers for capturing errors                                                                   // 147
var originalMeteorMethods = Meteor.methods;                                                                           // 148
Meteor.methods = function(methodMap) {                                                                                // 149
  _.each(methodMap, function(handler, name) {                                                                         // 150
    wrapMethodHanderForErrors(name, handler, methodMap);                                                              // 151
  });                                                                                                                 // 152
  originalMeteorMethods(methodMap);                                                                                   // 153
};                                                                                                                    // 154
                                                                                                                      // 155
                                                                                                                      // 156
function wrapMethodHanderForErrors(name, originalHandler, methodMap) {                                                // 157
  methodMap[name] = function() {                                                                                      // 158
    try{                                                                                                              // 159
      return originalHandler.apply(this, arguments);                                                                  // 160
    } catch(ex) {                                                                                                     // 161
      if(Kadira._getInfo()) {                                                                                         // 162
        Kadira._getInfo().currentError = ex;                                                                          // 163
                                                                                                                      // 164
        var newError = cloneError(ex);                                                                                // 165
        ex = newError;                                                                                                // 166
      }                                                                                                               // 167
      throw ex;                                                                                                       // 168
    }                                                                                                                 // 169
                                                                                                                      ...
                                                                                                                      // 172
function cloneError(err) {                                                                                            // 173
  if(err instanceof Meteor.Error) {                                                                                   // 174
    var newError = err.clone();                                                                                       // 175
  } else {                                                                                                            // 176
    var newError = new Error(err.message);                                                                            // 177
  }                                                                                                                   // 178
                                                                                                                      // 179
  newError.stack = {stack: err.stack, source: 'method'};                                                              // 180
  return newError;                                                                                                    // 181
}                                                                                                                     // 182
                                                                                                                      // 183
////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
