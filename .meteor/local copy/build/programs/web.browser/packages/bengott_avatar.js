...
Template["avatar"] = new Template("Template.avatar", (function() {                                 // 3
  ...
  return HTML.DIV({                                                                                // 5
    "class": function() {                                                                          // 6
      return [ "avatar ", Spacebars.mustache(view.lookup("size")), " ", Spacebars.mustache(view.lookup("shape")), " ", Spacebars.mustache(view.lookup("class")) ];
    }                                                                                              // 8
    ...
    alt: "avatar",                                                                                 // 14
    onerror: "this.style.display='none';"                                                          // 15
  }, function() {                                                                                  // 16
    return Spacebars.attrMustache(view.lookup("dimensions"));                                      // 17
  })), "\n    ", HTML.SPAN({                                                                       // 18
    "class": "avatar-initials",                                                                    // 19
    style: function() {                                                                            // 20
      return Spacebars.mustache(view.lookup("initialsCss"));                                       // 21
    }                                                                                              // 22
  }, Blaze.View("lookup:initialsText", function() {                                                // 23
    return Spacebars.mustache(view.lookup("initialsText"));                                        // 24
  })), "\n  ");                                                                                    // 25
}));                                                                                               // 26
                                                                                                   // 27
/////////////////////////////////////////////////////////////////////////////////////////////////////
...
Template.avatar.helpers({                                                                          // 1
                                                                                                   // 2
  size: function () {                                                                              // 3
    var valid = ['large', 'small', 'extra-small'];                                                 // 4
    return _.contains(valid, this.size) ? 'avatar-' + this.size : '';                              // 5
  },                                                                                               // 6
                                                                                                   // 7
  dimensions: function () {                                                                        // 8
    var value;                                                                                     // 9
    if      (this.size === 'large')       value = 80;                                              // 10
    else if (this.size === 'small')       value = 30;                                              // 11
    else if (this.size === 'extra-small') value = 20;                                              // 12
    else                                  value = 50;                                              // 13
                                                                                                   // 14
    return { width: value, height: value };                                                        // 15
  },                                                                                               // 16
                                                                                                   // 17
  shape: function () {                                                                             // 18
    var valid = ['rounded', 'circle'];                                                             // 19
    return _.contains(valid, this.shape) ? 'avatar-' + this.shape : '';                            // 20
  },                                                                                               // 21
                                                                                                   // 22
  class: function () { return this.class; },                                                       // 23
                                                                                                   // 24
  imageUrl: function () {                                                                          // 25
    var user = this.user ? this.user : Meteor.users.findOne(this.userId);                          // 26
    return Avatar.getUrl(user);                                                                    // 27
  },                                                                                               // 28
                                                                                                   // 29
  initialsCss: function () {                                                                       // 30
    var css = '';                                                                                  // 31
    if (this.bgColor)  css += 'background-color: ' + this.bgColor + ';';                           // 32
    if (this.txtColor) css += 'color: ' + this.txtColor + ';';                                     // 33
    return css;                                                                                    // 34
  },                                                                                               // 35
                                                                                                   // 36
  initialsText: function () {                                                                      // 37
    var user = this.user ? this.user : Meteor.users.findOne(this.userId);                          // 38
    return this.initials || Avatar.getInitials(user);                                              // 39
  }                                                                                                // 40
                                                                                                   // 41
});                                                                                                // 42
                                                                                                   // 43
  ...
  getUrl: function (user) {                                                                        // 73
    ...
    if (user) {                                                                                    // 78
      ...
      if (svc === 'twitter') {                                                                     // 80
        ...
        url = user.services.twitter.profile_image_url_https.replace('_normal.', '_200x200.');      // 82
      }                                                                                            // 83
      else if (svc === 'facebook') {                                                               // 84
        // use larger image (~200x200)                                                             // 85
        url = 'https://graph.facebook.com/' + user.services.facebook.id + '/picture?type=large';   // 86
      }                                                                                            // 87
      else if (svc === 'google') {                                                                 // 88
        url = user.services.google.picture;                                                        // 89
      }                                                                                            // 90
      else if (svc === 'github') {                                                                 // 91
        url = 'https://avatars.githubusercontent.com/' + user.services.github.username + '?s=200'; // 92
      }                                                                                            // 93
